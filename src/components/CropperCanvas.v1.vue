...
function drawAll(): void {
  ...
  if (firstAnchor.value && !secondAnchor.value && mousePosition.value) {
    ...
    if (converted.x > 0 && converted.y > 0) {
      ...
      ctx.setLineDash([5, 5])
      ctx.beginPath()
      ctx.moveTo(firstAnchor.value.x * drawScale.value + offset.value.x, firstAnchor.value.y * drawScale.value + offset.value.y)
      ctx.lineTo(converted.x * drawScale.value + offset.value.x, converted.y * drawScale.value + offset.value.y)
      ctx.stroke()
      ctx.restore()
...
function drawImage(ctx: CanvasRenderingContext2D): void {
  ...
  img.onload = () => {
    ...
    // 最简单的图片绘制（因为我们已经在canvas尺寸计算时缩放过了）
    ctx.drawImage(img, offset.value.x, offset.value.y, width, height)
  }
  img.src = imageInfo.value.src
}
